...
async function deletePart() {
  try {
    const result = await $api(
      "/master/makers/" + encodeURIComponent(selectedMakerCode.value),
      {
        method: "DELETE",
...
async function handleExport() {
  ...
  try {
    ...
    const response = await axios.get("/api/master/makers/export", {
      responseType: "blob",
      headers: accessToken
        ? {
      ...
      <template #item.actions="{ item }">
        <div class="align-center">
          <IconBtn
            ...
            @click="openEditPartPage(item.makercode)"
          >
            <VIcon icon="tabler-edit" />
          </IconBtn>
          <IconBtn
            v-if="$can('delete', 'masterData')"
            @click="openDeleteDialog(item.makercode)"
          >
