<script setup>
import AddMeasureDrawer from "@/components/drawers/AddMeasureDrawer.vue";
...
async function deleteItem() {
  try {
    const result = await $api("/master/measures/" + selectedId.value, {
      method: "DELETE",
...
async function handleExport() {
  ...
  try {
    ...
    const response = await axios.get("/api/master/measures/export", {
      responseType: "blob",
      headers: accessToken
        ? {
...
<template>
  ...
  <VCard class="mb-6">
    ...
    >
      <!-- part name -->
      <template #item.measurecode="{ item }">
        <div class="d-flex align-center">
          <span
            class="d-block font-weight-medium text-high-emphasis text-truncate"
            >{{ item.measurecode }}</span
          >
        </div>
      </template>

      <!-- date -->
      <template #item.measurename="{ item }">
        <div class="d-flex align-center">
          {{ item.measurename }}
        </div>
      </template>
