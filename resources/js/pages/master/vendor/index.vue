...
async function handleExport() {
  ...
  try {
    ...
    const response = await axios.get("/api/master/vendors/export", {
      responseType: "blob",
      headers: accessToken
        ? {
...
<template>
  ...
  <VCard class="mb-6">
    ...
    <VDataTable
      ...
      :items="data"
      :headers="headers"
      :sort-by="[{ key: 'vendorcode', order: 'asc' }]"
      class="text-no-wrap"
    >
      <!-- part name -->
      ...
      <template #item.actions="{ item }">
        <div class="align-center">
          <IconBtn
            ...
            @click="openEditPage(item.vendorcode)"
          >
            <VIcon icon="tabler-edit" />
          </IconBtn>
          <IconBtn
            v-if="$can('delete', 'masterData')"
            @click="openDeleteDialog(item.vendorcode)"
          >
